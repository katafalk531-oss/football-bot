...
TODAY_PARAMS = {"date": "2026-08-26", "status": "NS"}
...
async def get_today_matches(session):
    async with session.get(TODAY_API, headers=TODAY_HEADERS, params=TODAY_PARAMS) as resp:
        data = await resp.json()
...
async def cmd_signal(message: types.Message):
    ...
    text += "⚠️ Это value + xG + исторический H2H. Играйте ответственно."
